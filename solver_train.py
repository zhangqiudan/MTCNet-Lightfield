...
from dataset_lf import get_loader
...
class Solver(object):
    def __init__(self, train_loader, test_loader, config, save_fold=None):
        ...
        if config.mode == 'train':
            self.log_output = open("%s/logs_lf/log.txt" % config.save_fold, 'w')
        else:
    ...
    def train(self):
        ...
        for epoch in range(self.config.epoch):
            r_edge_loss, r_depth_loss, r_sal_loss, r_sum_loss = 0, 0, 0, 0
            self.net_bone.zero_grad()
            for i, data_batch in enumerate(self.train_loader):


                sal_image, sal_image90, sal_image0, sal_image45, sal_imageM45, sal_depth, sal_label, sal_edge = data_batch['sal_image'],data_batch['sal_image90'],data_batch['sal_image0'],data_batch['sal_image45'],data_batch['sal_imageM45'],data_batch['sal_depth'], data_batch['sal_label'], data_batch['sal_edge']
                #    continue
                sal_image, sal_image90, sal_image0, sal_image45, sal_imageM45, sal_depth, sal_label, sal_edge = Variable(sal_image),Variable(sal_image90),Variable(sal_image0),Variable(sal_image45),Variable(sal_imageM45),Variable(sal_depth), Variable(sal_label), Variable(sal_edge)
                if self.config.cuda:
                    sal_image, sal_image90, sal_image0, sal_image45, sal_imageM45, sal_depth, sal_label, sal_edge = sal_image.cuda(), sal_image90.cuda(),sal_image0.cuda(),sal_image45.cuda(),sal_imageM45.cuda(),sal_depth.cuda(),sal_label.cuda(), sal_edge.cuda()

                up_depth, up_edge, up_sal, up_sal_final = self.net_bone(sal_image, sal_image90, sal_image0, sal_image45, sal_imageM45)
                ...
                depth_loss =[]
                for ix in up_depth:
                    depth_loss.append(F.smooth_l1_loss(ix, sal_depth, size_average=True))
                depth_loss = sum(depth_loss) / (nAveGrad * self.config.batch_size)
                r_depth_loss += depth_loss.data
                ...
                loss = sal_loss + depth_loss + edge_loss
                ...
                if i % showEvery == 0:
                    print('epoch: [%2d/%2d], iter: [%5d/%5d]  ||  Depth : %10.4f ||  Sal : %10.4f  ||  Sum : %10.4f' % (
                        epoch, self.config.epoch, i, iter_num,
                        r_depth_loss * (nAveGrad * self.config.batch_size) / showEvery,
                ...
                if i % 200 == 0:
                    vutils.save_image(torch.sigmoid(up_sal_final[-1].data), tmp_path + '/iter%d-sal-0.jpg' % i,
            ...
            if (epoch + 1) % self.config.epoch_save == 0:
                torch.save(self.net_bone.state_dict(),
                           '%s/model/epoch_%d_bone.pth' % (self.config.save_fold, epoch + 1))
        ...
        torch.save(self.net_bone.state_dict(), '%s/model/final_bone.pth' % self.config.save_fold)
